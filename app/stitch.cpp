...
main(int argc, char **argv)
{
    ...
    string image_path   = std::get<string>(Parser.get(Options::OPT_IMAGE_PATH      ));
    ...
    if (mode == "image")
    {
        ...
        if (img.empty())
        {
            CV_Error_(cv::Error::StsBadArg, ("Error opening image: %s", image_path.c_str()));
        }

        // Enforce expected dual-fisheye resolution for current MLS grid (3840x1920)
        if (img.cols != 3840 || img.rows != 1920)
        {
            std::cout << "Resizing input to 3840x1920 to match MLS grid..\n";
            cv::resize(img, img, cv::Size(3840, 1920), 0, 0, cv::INTER_AREA);
        }
        ...
        cv::Mat img_l = img(cv::Rect(0, 0, frame_width / 2, frame_height));
        cv::Mat img_r = img(cv::Rect(frame_width / 2, 0, frame_width / 2, frame_height));

        cv::Mat pano = Stitcher.stitch(img_l, img_r);

        if (!std::filesystem::exists(out_dir))
        {
            std::filesystem::create_directories(out_dir);
        }
        std::string out_path = out_dir + "/" + image_name + "_blend.jpg";
        cv::imwrite(out_path, pano);
        std::cout << "Done! Wrote image --> " << out_path << "\n";
        return 0;
    }
    else if (mode == "video")
    {
    ...
    {
        CV_Error_(cv::Error::StsBadArg, ("Unknown mode: %s", mode.c_str()));
    }
}